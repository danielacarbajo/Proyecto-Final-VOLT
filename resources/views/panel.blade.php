...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8 flex items-start justify-between">
        <div>
            <h1 class="font-bebas text-4xl text-[#0f172a] tracking-wide">
    ...
    @if ($sinActividad)
        ...
        <div class="bg-gradient-to-br from-[#0f172a] to-[#1e293b] text-white rounded-2xl p-10 text-center">
            ...
            <h2 class="font-bebas text-3xl mb-2 tracking-wide">¿EMPEZAMOS?</h2>
    ...
    @else
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-[#ffd600] border border-[#eab308] rounded-2xl p-5 shadow-sm hover:shadow-md transition">
                <div class="flex items-start gap-3 mb-2">
                    <div class="w-11 h-11 rounded-xl bg-[#0f172a] flex items-center justify-center flex-shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#ffd600]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="font-bebas text-5xl text-[#0f172a] leading-none">{{ $entrenamientosMes }}</div>
                <div class="text-xs text-[#854d0e] font-medium mt-2 flex items-center gap-1">
                    @if ($diferenciaSemana > 0)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 10l7-7m0 0l7 7m-7-7v18"/>
                        </svg>
                        +{{ $diferenciaSemana }} vs semana pasada
                    @elseif ($diferenciaSemana < 0)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3"/>
                        </svg>
                        {{ $diferenciaSemana }} vs semana pasada
                    @else
                        {{ $entrenamientosMes === 1 ? 'sesión este mes' : 'sesiones este mes' }}
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition">
                ...
                <div class="text-xs text-neutral-400 mt-2">creadas</div>
            </div>
            ...
            <div class="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition">
                ...
                <div class="text-xs text-neutral-400 mt-2">en tu catálogo</div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
            ...
            <div class="lg:col-span-2 bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
                <div class="flex items-center justify-between mb-2">
                    <div>
                        <h2 class="font-bebas text-2xl text-[#0f172a] tracking-wide">ACTIVIDAD SEMANAL</h2>
                        <p class="text-xs text-neutral-500 mt-0.5">
                            {{ $entrenamientosSemanaActual }}
                            {{ $entrenamientosSemanaActual === 1 ? 'entrenamiento' : 'entrenamientos' }}
                            esta semana
                        </p>
                    </div>
            ...
            <div class="space-y-4">
                ...
                @if ($ultimoEntrenamiento)
                    ...
                    <div class="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm">
                        ...
                        {{-- Fecha --}}
                        <div class="font-bebas text-2xl text-[#0f172a] leading-none">
                        ...
                        @if ($ultimoEntrenamiento->rutina)
                            <div class="mt-3">
                                <span class="bg-[#ffd600]/20 text-[#854d0e] px-2 py-0.5 rounded-full text-xs font-medium">
                                    {{ $ultimoEntrenamiento->rutina->nombre }}
                                </span>
                            </div>
                        @endif

                        {{-- Mini-stats: ejercicios + series --}}
                        <div class="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-neutral-100">
                            <div>
                                <div class="font-bebas text-xl text-[#0f172a] leading-none">{{ $ultimoEntrenoEjercicios }}</div>
                                <div class="text-[10px] uppercase tracking-wider text-neutral-500 mt-1">
                                    {{ $ultimoEntrenoEjercicios === 1 ? 'Ejercicio' : 'Ejercicios' }}
                                </div>
                            </div>
                            <div>
                                <div class="font-bebas text-xl text-[#0f172a] leading-none">{{ $ultimoEntrenoSeries }}</div>
                                <div class="text-[10px] uppercase tracking-wider text-neutral-500 mt-1">
                                    {{ $ultimoEntrenoSeries === 1 ? 'Serie total' : 'Series totales' }}
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center justify-between mb-3">
            <h2 class="font-bebas text-2xl text-[#0f172a] tracking-wide">ACCESOS RÁPIDOS</h2>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
            <a href="{{ route('entrenamientos.create') }}"
               class="group bg-[#0f172a] hover:bg-[#1e293b] text-white rounded-2xl p-5 transition flex flex-col items-center gap-2 text-center">
                <div class="w-12 h-12 rounded-xl bg-[#ffd600] flex items-center justify-center group-hover:scale-110 transition">
            ...
            <a href="{{ route('entrenamientos.index') }}"
               class="group bg-white border border-neutral-200 hover:border-[#ffd600] hover:shadow-md rounded-2xl p-5 transition flex flex-col items-center gap-2 text-center">
                <div class="w-12 h-12 rounded-xl bg-[#ffd600]/20 flex items-center justify-center group-hover:scale-110 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#854d0e]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-neutral-500 mt-0.5">
                                    @if ($entreno->rutina)
                                        <span class="bg-[#ffd600]/20 text-[#854d0e] px-2 py-0.5 rounded-full font-medium">
